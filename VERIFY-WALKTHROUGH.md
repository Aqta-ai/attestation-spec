...
- Add any thirteenth top-level field: `valid: false` under strict mode (the spec forbids
  extra fields in v1).
- Run the conformance vectors in [CONFORMANCE.md](./CONFORMANCE.md): 25 vectors, 10 valid and
  15 deliberately invalid (wrong signature, mutated fields, malformed hashes, unknown fields,
  unsorted policies, malformed timestamps, negative costs). A conformant verifier must agree
  on all 25, and [CONFORMANCE-REPORT.md](./CONFORMANCE-REPORT.md) is generated by a script
  that fails on divergence rather than asserted by hand, so you can regenerate it yourself.
- There is also an in-browser verifier at [app.aqta.ai/verify](https://app.aqta.ai/verify)
  that runs the same check client-side and pre-verifies a signed sample on load.
