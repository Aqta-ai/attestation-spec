...
#
# 1.2.5 carries the policy_applied ordering fix. The Python package is the one
# that changes behaviour, so a PyPI upload that does not happen leaves the
# reported divergence live for every pip install. npm alone is not the fix.
#
#   npm login                       # or export NPM_TOKEN
#   export TWINE_USERNAME=__token__ # project-scoped PyPI token
#   export TWINE_PASSWORD=pypi-...
#   bash scripts/release-1.2.5.sh
set -euo pipefail
cd "$(dirname "$0")/.."
