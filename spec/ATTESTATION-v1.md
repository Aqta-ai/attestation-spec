...
```json
{
  ...
  "public_key": "UPfopwYcG6sZ03Z9htWmLQE0lHRGWnBNV2c_QiukxJ0"
}
```
...
- **Errata (2026-08-24).** Three corrections, none of which change the wire
  ...
  - §8's examples no longer show a real public key. They previously showed the
    ...
    where an implementer copies a pin from. The example key is now derived from
    the published seed `sha256("attestation-spec/example-key-not-for-pinning")`,
    so it has a valid Ed25519 shape, can be regenerated by anyone, and is
    self-evidently not a production key. Fetch the authoritative key from the
    published key endpoint; never from a document.
